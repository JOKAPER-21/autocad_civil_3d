...
3. A dialog appears with:
   - **Yard** (boxed section) — the yard list, organized by segment.
     ...
     automatically jumps to the nearest yard row instead. The line
     underneath the list updates live to show exactly where the
     selected yard will publish to. **Pressing Enter accepts the
     dialog the same as clicking OK** (OK is the dialog's default
     button — standard Windows-dialog behavior, triggered from any
     control).
...
2. The drawing is **saved** (`QSAVE`) under its **current** name — it
   keeps its original name and stays open exactly as before; nothing
   about your session changes yet.
