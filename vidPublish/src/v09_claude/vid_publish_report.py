...
# --------------------------------------------------------------------------
ROOT_PATH = r"\\Desktop-igi8ekn\vid_20d\share\updated_projects_files\section"
# --------------------------------------------------------------------------
...
#   timestamp,computer_name,code,filename,srcpath,destdwg,pdfresult,destpdf,
#   commands,newversion,newfilever,verfolder
#
#   computer_name is the PUBLISHER'S PC SYSTEM NAME (Windows
#   %COMPUTERNAME% / hostname) -- NOT their AutoCAD/Windows login name.
#   This is what shows up if you run "hostname" in a terminal on that
#   machine, e.g. "DESKTOP-7F3K2QX" or "JDOE-LAPTOP".
#
...
#
#   newversion is "yes" or "no" -- whether the "New Version" checkbox was
#   checked for that publish.
#   newfilever is the drawing's OWN bumped filename version number (e.g.
#   "3" if the file became ..._v03), or "" if newversion was "no" or the
#   file had no version pattern handling applied.
#   verfolder is the SERVER-SIDE version folder name created for that
#   publish (e.g. "v06"), or "" if newversion was "no". This number is
#   tracked INDEPENDENTLY of newfilever -- they commonly differ, since
#   one counts the file's own version tag and the other counts existing
#   version folders already on the server for that yard.
# --------------------------------------------------------------------------

LOG_FIELDS = ["timestamp", "computer_name", "code", "filename", "srcpath",
              "destdwg", "pdfresult", "destpdf", "commands",
              "newversion", "newfilever", "verfolder"]
NUM_FIELDS = len(LOG_FIELDS)
COMMANDS_INDEX = LOG_FIELDS.index("commands")          # fields before this are fixed, front-anchored
TAIL_FIELDS_AFTER_COMMANDS = LOG_FIELDS[COMMANDS_INDEX + 1:]  # fixed, back-anchored
    ...
    with open(candidate, "r", encoding="utf-8", newline="") as f:
        ...
        for raw in reader:
            ...
            if len(raw) > NUM_FIELDS:
                ...
                # "commands" column, which sits in the MIDDLE of the row
                # (not first, not last). Anchor the fixed fields BEFORE
                # commands from the front, and the fixed fields AFTER
                # commands from the back, then re-glue whatever's left
                # in the middle into a single commands string.
                n_tail = len(TAIL_FIELDS_AFTER_COMMANDS)
                head = raw[:COMMANDS_INDEX]
                tail = raw[len(raw) - n_tail:] if n_tail else []
                middle = raw[COMMANDS_INDEX: len(raw) - n_tail] if n_tail else raw[COMMANDS_INDEX:]
                commands_joined = ",".join(middle)

                row = dict(zip(LOG_FIELDS[:COMMANDS_INDEX], head))
                row["commands"] = commands_joined
                row.update(dict(zip(TAIL_FIELDS_AFTER_COMMANDS, tail)))
                rows.append(row)
...
def build_yard_status(root: str, log_rows: list):
    """For every yard in YARD_MAP, compute:
        ...
        - last_was_new_version (bool -- was "New Version" checked on the
          most recent publish for this yard)
        - last_file_version (the drawing's own bumped version number from
          the most recent "New Version" publish, or None)
        - last_version_folder (the server-side version folder name, e.g.
          "v06", from the most recent "New Version" publish, or None)
        - version_publish_count (how many of this yard's publishes had
          "New Version" checked, out of publish_count total)
    ...
    for code, segment in YARD_MAP:
        ...
        was_new_version = (last.get("newversion") == "yes") if last else False
        last_file_version = last.get("newfilever") if was_new_version else None
        last_version_folder = last.get("verfolder") if was_new_version else None
        version_publish_count = sum(1 for r in rows if r.get("newversion") == "yes")
        ...
        status.append({
            ...
            "last_computer": last["computer_name"] if last else None,
            "last_dest": last_dest,
            ...
            "last_was_new_version": was_new_version,
            "last_file_version": last_file_version,
            "last_version_folder": last_version_folder,
            "version_publish_count": version_publish_count,
    ...
    if not missing_only:
        ...
        for s in published:
            ...
            print(f"  {s['code']:<26} count={s['publish_count']:<3} "
                  f"last={s['last_published']:<16} from={s['last_computer']:<16} "
                  f"[DWG:{dwg_flag}] [PDF:{pdf_flag}]")
            if s["last_commands"]:
                print(f"      commands: {s['last_commands']}")
            if s["last_was_new_version"]:
                fv = s["last_file_version"] or "?"
                vf = s["last_version_folder"] or "?"
                print(f"      new version: file -> v{fv.zfill(2) if fv != '?' else fv}, "
                      f"server folder -> {vf} "
                      f"(versioned {s['version_publish_count']}/{s['publish_count']} times)")
...
def write_csv(status: list, out_path: str):
    fieldnames = ["code", "segment", "publish_count", "last_published",
                  "last_computer", "last_dest", "last_dest_exists",
                  "last_pdf_result", "last_pdf_path", "last_pdf_exists",
                  "last_commands", "last_was_new_version", "last_file_version",
                  "last_version_folder", "version_publish_count",
                  "path_styles_seen", "target_folder"]
