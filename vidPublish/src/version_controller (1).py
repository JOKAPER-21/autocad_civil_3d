...
MONO_FONT_SM = ("Consolas", 8)
SANS_FONT = ("Segoe UI", 9)
SANS_FONT_SM = ("Segoe UI", 8)
SANS_BOLD = ("Segoe UI", 9, "bold")
SANS_BOLD_SM = ("Segoe UI", 8, "bold")
...
class FilterPopover(tk.Toplevel):
    """
    A proper Toplevel placed and raised explicitly (lift + focus_force + grab_set)
    so Windows always shows it correctly positioned relative to its trigger
    button -- this fixes the previous "dropdown not showing" issue.
    """
    ...
    def _build(self, column):
        ...
        canvas = tk.Canvas(list_outer, bg=PANEL_BG, highlightthickness=0, height=210, width=220)
        ...
        canvas.create_window((0, 0), window=self.list_frame, anchor="nw", width=220)
        ...
        cancel_btn = tk.Button(foot, text="Cancel", bg=PANEL_BG, fg=TEXT_DIM, relief="flat",
                                ...
                                activebackground=PANEL_BG_2, bd=0, padx=10, pady=4)
...
class VersionControllerApp:
    ...
    def _build_ui(self):
        ...
        stamp = tk.Label(left, text="REV", bg=AMBER_BG, fg=AMBER, font=("Consolas", 9, "bold"),
        ...
        self.counter_label.pack(side="left", padx=(0, 14))
        ...
        toolbar = tk.Frame(self.root, bg=PANEL_BG_2, height=48)
        toolbar.pack(fill="x", side="top")
        toolbar.pack_propagate(False)
        tk.Frame(self.root, bg=LINE, height=1).pack(fill="x", side="top")

        search_frame = tk.Frame(toolbar, bg="#ffffff", highlightbackground=LINE, highlightthickness=1)
        search_frame.pack(side="left", padx=12, pady=9, ipady=2)
        tk.Label(search_frame, text="Search:", bg="#ffffff", fg=TEXT_DIM, font=SANS_FONT_SM).pack(side="left", padx=(8, 4))
        self.search_var = tk.StringVar()
        search_entry = tk.Entry(search_frame, textvariable=self.search_var, bg="#ffffff", fg=TEXT,
        ...
        filter_scroll_outer.pack(side="left", padx=4, pady=9, fill="x", expand=True)
        ...
        right_tools.pack(side="right", padx=12, pady=9)
        ...
        tk.Label(footer, text="Click Open next to a path to launch it in File Explorer",
    ...
    def _make_btn(self, parent, text, command, primary=False):
        if primary:
            ...
            btn = tk.Button(parent, text=text, command=command, bg="#ffffff", fg=TEXT,
                             ...
                             bd=1, highlightbackground=LINE, padx=11, pady=4)
        return btn
    ...
    def _compute_col_widths(self):
        ...
        for c in self.columns:
            if c.is_path:
                self.col_widths[c.key] = 260
            elif c.is_long_text:
                self.col_widths[c.key] = 220
            elif c.is_date:
                self.col_widths[c.key] = 150
            elif c.is_status:
                self.col_widths[c.key] = 110
            else:
                self.col_widths[c.key] = 130
    ...
    def _build_header_row(self):
        for w in self.header_frame.winfo_children():
            w.destroy()
        for c in self.columns:
            cell = tk.Frame(self.header_frame, bg=HEADER_BG, width=self.col_widths[c.key], height=32,
            ...
            lbl.pack(fill="both", expand=True, padx=8, pady=6)
    ...
    def _render_chips(self):
        ...
        for field, vals in self.filters.items():
            ...
            for v in vals:
                ...
                tk.Label(chip, text=lbl, bg=AMBER_BG, fg=AMBER, font=("Consolas", 8)).pack(side="left", padx=(8, 4), pady=3)
                x = tk.Label(chip, text="\u2715", bg=AMBER_BG, fg=AMBER, font=("Consolas", 8, "bold"), cursor="hand2")
    ...
    def render(self):
        ...
        for i, r in enumerate(rows):
            ...
            for c in self.columns:
                width = self.col_widths[c.key]
                cell = tk.Frame(row_frame, bg=bg, width=width, height=30,
                                 highlightbackground=LINE_SOFT, highlightthickness=1)
                cell.pack(side="left", fill="y")
                cell.pack_propagate(False)
    ...
    def _render_cell(self, cell, column, value, bg):
        if column.is_path:
            ...
            text_lbl = tk.Label(inner, text=short_path(value) or "\u2014", bg=bg, fg=TEXT_DIM,
                                 font=MONO_FONT_SM, anchor="w")
            text_lbl.pack(side="left", fill="both", expand=True)

            if value:
                open_btn = tk.Button(inner, text="Open", command=lambda v=value: self.open_in_explorer(v),
                                      bg="#ffffff", fg=ACCENT, relief="solid", bd=1,
                                      highlightbackground=LINE, font=("Segoe UI", 7, "bold"),
                                      cursor="hand2", padx=8, pady=1,
                                      activebackground=AMBER_BG, activeforeground=ACCENT)
        ...
        if re.search(r"yard|code", column.key, re.I):
            ...
            tag = tk.Label(inner, text=value or "\u2014", bg=PANEL_BG_2, fg=TEXT_DIM,
                            font=MONO_FONT_SM, padx=6, pady=1)
            tag.pack(side="left")
            return
