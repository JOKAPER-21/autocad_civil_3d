...
5. If **Generate PDF** is checked, the current layout is plotted to PDF
   (DWG To PDF.pc3) into the **same subfolder**, also with the same base
   name. If unchecked, the subfolder is still created — it just won't
   contain a PDF.
...
- **PDF plot fails**: the DWG still publishes successfully; you get a
  warning, and the log records `pdfresult = failed` for that entry so it
  shows up in the Python report too.

## 3. About the DWGPROPS command
...
| `username` | Windows login name (`LOGINNAME` system variable) |
