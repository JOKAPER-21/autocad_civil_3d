#!/usr/bin/env python3
"""
yard_publish_report.py
-----------------------
Companion script for YardPublish.lsp (AutoCAD Civil 3D 2026 pipeline).

WHAT THIS DOES
    The LSP does all the AutoCAD-side work (save, copy, log). This script
    does NOT touch AutoCAD or DWG files. It reads:
    ...
    and the actual folder structure under <ROOT_PATH>, then reports:
        ...
        - the most recent publish per yard
        - yards that have NEVER been published (still pending)
        - log entries whose destination file no longer exists on disk
...
USAGE
    ...
    python yard_publish_report.py "\\\\server\\share\\VID_autocad_publish_test"
...
This script is read-only with respect to the publish log and DWG files.
It never modifies, moves, or deletes anything in ROOT_PATH.
"""
...
# Same root path as *YP-ROOT-PATH* in YardPublish.lsp.
...
# Mirrors *YP-YARD-MAP* in YardPublish.lsp exactly.
...
#   timestamp,username,code,filename,srcpath,destpath
# srcpath and destpath themselves contain backslashes but no commas in
# normal use (Windows paths don't use commas). We still parse defensively:
# if a row has MORE than 6 fields, we assume the extra commas were inside
# srcpath/destpath joins (shouldn't happen, but filenames with commas in
# the original DWG name could cause it) and re-glue conservatively.
# --------------------------------------------------------------------------

LOG_FIELDS = ["timestamp", "username", "code", "filename", "srcpath", "destpath"]
...
def read_publish_log(root: str):
    ...
    """
    log_path = Path(norm_win_path(root)) if os.name == "nt" else None
    # Build path string manually too, since on non-Windows hosts Path()
    ...
    with open(candidate, "r", encoding="utf-8", newline="") as f:
        ...
        for raw in reader:
            ...
            if len(raw) < 6:
                ...
                continue
            if len(raw) > 6:
                # Re-glue: keep first 4 fields as-is, join the remainder
                # back into srcpath, then split srcpath/destpath on the
                # LAST comma only (destpath is the final field).
                head = raw[:4]
                tail_joined = ",".join(raw[4:])
                # destpath was the last field written; find the last
                # comma that separates srcpath from destpath is unsafe
                # in general, so we just keep the joined remainder as
                # destpath and leave srcpath blank rather than guessing.
                rows.append(dict(zip(LOG_FIELDS[:4], head),
                                  srcpath="(unparsed - see destpath)",
                                  destpath=tail_joined))
                print(f"[WARN] Log line had extra commas, parsed leniently: {raw}")
                continue
...
def build_yard_status(root: str, log_rows: list):
    """For every yard in YARD_MAP, compute:
        ...
        - last_dest (path string, or None)
        - last_dest_exists_on_disk (bool or 'unknown' if not checkable here)
        - target_folder (expected publish folder)
        - path_style_seen (set of styles seen in log destpaths for this yard)
    """
    ...
    for code, segment in YARD_MAP:
        ...
        last_dest = last["destpath"] if last else None
        exists = "unknown"
        if last_dest:
            resolved = _resolve_existing_path_any(last_dest)
            exists = resolved is not None

        styles_seen = sorted({path_style(r.get("destpath", "")) for r in rows}) if rows else []
...
def print_summary(status: list, missing_only: bool):
    ...
    print("=" * 78)
    print(f"YARD PUBLISH REPORT  -  generated {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 78)
    ...
    if not missing_only:
        ...
        for s in published:
            ...
            print(f"  {s['code']:<26} count={s['publish_count']:<3} "
                  f"last={s['last_published']:<16} by={s['last_user']:<12} [{exist_flag}]")
