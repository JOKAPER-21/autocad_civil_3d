...
class FilterPopover(tk.Toplevel):
    def __init__(self, parent, app, column, anchor_widget):
        ...
        # Force the anchor widget to finalize its screen position before we
        # read winfo_rootx/y -- on Windows these return 0 on un-realized widgets
        ...
        self.lift()
        self.focus_force()
        # NOTE: do NOT call grab_set() here -- on Windows it forces the window
        # to appear at (0,0) before geometry takes effect, causing the
        # "top-right corner" bug.  We handle outside-click via bind_all instead.
...
class VersionControllerApp:
    ...
    def _build_ui(self):
        ...
        # ── Scrollable table area (header + body share one horizontal scrollbar) ──
        # Layout inside table_outer (grid):
        #   row=0: header_canvas   (fixed height, scrolls horizontally only)
        #   row=1: body_canvas     (expands, scrolls both axes)
        #   row=2: hsb             (shared horizontal scrollbar)
        #   col=1: vsb             (vertical scrollbar for body only)
        table_outer = tk.Frame(self.root, bg=APP_BG, highlightbackground=LINE, highlightthickness=1)
        table_outer.pack(fill="both", expand=True, padx=12, pady=(10, 10))

        vsb = ttk.Scrollbar(table_outer, orient="vertical")
        hsb = ttk.Scrollbar(table_outer, orient="horizontal")

        # Header canvas -- horizontally scrollable, fixed height
        self.header_canvas = tk.Canvas(table_outer, bg=HEADER_BG, highlightthickness=0, height=36)
        self.header_frame = tk.Frame(self.header_canvas, bg=HEADER_BG)
        self._hdr_win = self.header_canvas.create_window((0, 0), window=self.header_frame, anchor="nw")
        self.header_frame.bind(
            "<Configure>",
            lambda e: self.header_canvas.configure(scrollregion=self.header_canvas.bbox("all"))
        )

        # Body canvas -- scrolls both axes
        self.body_canvas = tk.Canvas(table_outer, bg=PANEL_BG, highlightthickness=0)

        # Wire the shared horizontal scrollbar to move BOTH canvases together
        def _xview(*args):
            self.header_canvas.xview(*args)
            self.body_canvas.xview(*args)
        hsb.configure(command=_xview)

        def _header_xscroll(first, last):
            hsb.set(first, last)
            self.body_canvas.xview_moveto(first)
        def _body_xscroll(first, last):
            hsb.set(first, last)
            self.header_canvas.xview_moveto(first)

        self.header_canvas.configure(xscrollcommand=_header_xscroll)
        self.body_canvas.configure(yscrollcommand=vsb.set, xscrollcommand=_body_xscroll)
        vsb.configure(command=self.body_canvas.yview)

        self.header_canvas.grid(row=0, column=0, sticky="ew")
        tk.Frame(table_outer, bg=LINE, height=1).grid(row=1, column=0, columnspan=2, sticky="ew")
        self.body_canvas.grid(row=2, column=0, sticky="nsew")
    ...
    def _build_header_row(self):
        ...
        del_header = tk.Frame(self.header_frame, bg=HEADER_BG, width=80, height=36,
                               highlightbackground=LINE_SOFT, highlightthickness=1)
        del_header.pack(side="left", fill="y")
        del_header.pack_propagate(False)
        tk.Label(del_header, text="DELETE", bg=HEADER_BG, fg=RED,
                 font=SANS_BOLD_SM, anchor="center").pack(fill="both", expand=True, padx=6, pady=8)
        # Keep header_canvas height in sync with the header_frame content
        self.header_frame.update_idletasks()
        self.header_canvas.configure(height=self.header_frame.winfo_reqheight())
    ...
    def render(self):
        ...
        for i, r in enumerate(rows):
            ...
            # delete button cell (pinned to right of every data row)
            del_cell = tk.Frame(row_frame, bg=bg, width=80, height=34,
                                 highlightbackground=LINE_SOFT, highlightthickness=1)
            del_cell.pack(side="left", fill="y")
            del_cell.pack_propagate(False)
            del_btn = tk.Button(del_cell, text="\u2715 Del",
                                 command=lambda row=r: self.delete_entry(row),
                                 bg="#fbeae8", fg="#c1392b", relief="flat", bd=0,
                                 font=("Segoe UI", 9, "bold"), cursor="hand2", padx=8, pady=3,
                                 activebackground="#f5c6c2", activeforeground="#c1392b")
    ...
    def delete_entry(self, row):
        """
        Ask for confirmation, then remove this row from self.data and
        write the updated list back to the CSV file immediately.
        The row is identified by object identity (the dict in self.data),
        so there is no index confusion even after filtering/sorting.
        """
        # build a short summary for the confirm dialog from the first few fields
        summary_keys = self.raw_headers[:3]
        summary = "  \n".join(f"{k}: {row.get(k, '')}" for k in summary_keys)
        ...
        # find and remove by identity (not index) so filters don't cause a mismatch
        try:
            self.data.remove(row)
        except ValueError:
            messagebox.showerror("Delete failed", "Could not locate the entry in the data list.")
            return
    ...
    def _save_csv(self):
        """Write self.data back to the current CSV file, preserving column order."""
        if not self.current_csv_path:
            messagebox.showerror("Save failed", "No CSV file path is set.")
            return
        try:
            with open(self.current_csv_path, "w", newline="", encoding="utf-8-sig") as f:
                writer = csv.DictWriter(f, fieldnames=self.raw_headers, extrasaction="ignore")
                writer.writeheader()
                for r in self.data:
                    writer.writerow(r)
        except Exception as e:
